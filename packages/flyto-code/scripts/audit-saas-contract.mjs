...
const CAPABILITIES_RELATIVE_PATH = 'internal/permission/capabilities.yaml'
const CAPABILITIES_SNAPSHOT = path.join(SRC, 'lib/engine/__generated__/backend-capabilities.yaml')
...
function generatedCapabilitiesContent(raw) {
  return [
...
function assertSnapshotFresh(liveCapabilitiesPath) {
  const live = read(liveCapabilitiesPath)
  const expected = generatedCapabilitiesContent(live)
  const current = fs.existsSync(CAPABILITIES_SNAPSHOT)
    ? fs.readFileSync(CAPABILITIES_SNAPSHOT, 'utf8')
    : ''
  if (current !== expected) {
    fail('backend capability snapshot is stale; run `npm run sync:backend-capabilities`')
  }
}
...
function resolveCapabilitiesSource() {
  ...
  for (const candidate of engineRootCandidates()) {
    ...
    const capabilitiesPath = path.join(resolved, CAPABILITIES_RELATIVE_PATH)
    attempted.push(capabilitiesPath)
    if (fs.existsSync(capabilitiesPath)) {
      assertSnapshotFresh(capabilitiesPath)
      return { path: capabilitiesPath, label: `live engine ${path.relative(ROOT, capabilitiesPath)}` }
    }
  }

  if (fs.existsSync(CAPABILITIES_SNAPSHOT)) {
    warn(
      'using generated backend capability snapshot because live flyto-engine checkout is unavailable; ' +
      'run `npm run check:backend-capabilities` locally with flyto-engine present before updating the snapshot',
    )
    return { path: CAPABILITIES_SNAPSHOT, label: 'generated backend capability snapshot' }
  }

  fail(
    `missing backend capability registry (${CAPABILITIES_RELATIVE_PATH}); ` +
    `set FLYTO_ENGINE_ROOT to a flyto-engine checkout or run ` +
    `\`npm run sync:backend-capabilities\`. Tried: ${attempted.join(', ')}, ${CAPABILITIES_SNAPSHOT}`,
  )
  return { path: CAPABILITIES_SNAPSHOT, label: 'missing capability source' }
}

const CAPABILITIES_SOURCE = resolveCapabilitiesSource()
const CAPABILITIES_YAML = CAPABILITIES_SOURCE.path
...
function parseCapabilities() {
  if (!fs.existsSync(CAPABILITIES_YAML)) {
    exitWithFailures()
  }
  const raw = read(CAPABILITIES_YAML)

...
describe('ProductVerificationView', () => {
  ...
  it('uses a fixed shell with product-verification cockpit and testing matrix tabs', async () => {
    ...
    expect(screen.getByText('Authorized web security test')).toBeTruthy()
    expect(screen.getByText('Scan the site. See the proof. Fix with confidence.')).toBeTruthy()
    expect(screen.getByText('Start with a target')).toBeTruthy()
    expect(screen.getByText('Find pages and actions')).toBeTruthy()
    expect(screen.getByText('Replay key flows')).toBeTruthy()
    expect(screen.getByText('Capture proof')).toBeTruthy()
    expect(screen.getByText('Show pass or blocked')).toBeTruthy()
    expect(screen.getByText('How Flyto decides')).toBeTruthy()
    expect(screen.getByText('Approved targets')).toBeTruthy()
    expect(screen.getByText('Runs only use targets already authorized by the engine. Missing scope fails closed.')).toBeTruthy()
    expect(screen.getByText('Browser replay')).toBeTruthy()
    expect(screen.getByText('No AI shortcut')).toBeTruthy()
    expect(screen.getByText('Proof attached')).toBeTruthy()
    expect(screen.getAllByText('Target under verification').length).toBeGreaterThan(0)
    expect(screen.getByText('Customer target URL')).toBeTruthy()
    expect((await screen.findAllByText('https://checkout.customer.example')).length).toBeGreaterThan(0)
    expect(await screen.findByText('checkout.customer.example')).toBeTruthy()
    expect(await screen.findByText('repo-customer-shop')).toBeTruthy()
    expect(await screen.findByText('customer-owned URL/domain/repo')).toBeTruthy()
    expect(screen.getByText('Verifier provenance')).toBeTruthy()
    expect(screen.getByText('Control plane')).toBeTruthy()
    expect(screen.getByText('flyto-engine')).toBeTruthy()
    expect(screen.getByText('flyto-core verification runner')).toBeTruthy()
    ...
    expect(screen.getByText('2 scenario steps · 4 UI states')).toBeTruthy()
    expect(screen.getByText('2/2 replay steps · 100%')).toBeTruthy()
    expect(screen.getByText('screenshot · DOM · network · sha256:evidence')).toBeTruthy()
    expect(screen.getByText('flyto-i18n key gate · flyto-code hardcoded-English audit · build sync')).toBeTruthy()

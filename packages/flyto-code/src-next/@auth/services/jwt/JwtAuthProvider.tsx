...
function JwtAuthProvider(props: FuseAuthProviderComponentProps) {
	...
	// Token lives in sessionStorage so it's cleared on tab close and is
	// not exposed to other tabs (smaller XSS replay window than
	// localStorage). Auto-relogin within the same tab still works via
	// the access token; cross-tab persistence is intentionally dropped.
	const {
		value: tokenStorageValue,
		setValue: setTokenStorageValue,
		removeValue: removeTokenStorageValue
	} = useSessionStorage<string>('jwt_access_token');

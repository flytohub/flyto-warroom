...
export const themesConfig: FuseThemesType = {
	default: {
		palette: {
			...
			...neutralsLightTheme,
			text: { primary: '#1e293b', secondary: '#334155', disabled: '#64748b' },
			// True neutral background — operator complained repeatedly
			// that the previous `#eef1f5` (slate-200 territory) read
			// as "藍底" because slate has a cool blue undertone.
			// `#f5f5f5` is pure zinc (no chroma) so the page never
			// reads as tinted; gives enough contrast against the
			// `#ffffff` paper cards without feeling sterile like flat
			// white-on-white.
	...
	defaultNavbar: {
		palette: {
			mode: 'dark',
			primary: { main: '#a78bfa', light: '#c4b5fd', dark: '#7c3aed', contrastText: '#FFFFFF' },
			secondary: { main: '#22d3ee', light: '#67e8f9', dark: '#06b6d4', contrastText: '#FFFFFF' },
			text: { primary: '#f1f5f9', secondary: '#c4b5fd', disabled: '#7c6bc4' },
			background: { default: '#1e1045', paper: '#2a1557' },

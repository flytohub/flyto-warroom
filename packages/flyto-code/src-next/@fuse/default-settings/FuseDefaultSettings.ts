 ...
 *   1. localStorage (user's explicit choice persists across sessions)
...
const darkPalette = {
	...
	common: { black: 'rgb(17, 24, 39)', white: 'rgb(255, 255, 255)' },
	primary: { light: '#bec1c5', main: '#c084fc', dark: '#7c3aed', contrastDefaultColor: 'light' },
	secondary: { light: '#bdf2fa', main: '#22d3ee', dark: '#0cb7e2' },
	background: { paper: '#1E2125', default: '#121212' },
	error: { light: '#ffcdd2', main: '#f44336', dark: '#b71c1c' },
};
...
const lightPalette = {
	...
	common: { black: 'rgb(17, 24, 39)', white: 'rgb(255, 255, 255)' },
	primary: { light: '#bec1c5', main: '#252f3e', dark: '#0d121b', contrastDefaultColor: 'light' },
	secondary: { light: '#bdf2fa', main: '#22d3ee', dark: '#0cb7e2' },
 ...
 * Respects system preference + localStorage persistence.
 */
const defaultTheme = {
	palette: initialMode === 'dark' ? darkPalette : lightPalette,
};
...
export const defaultThemeOptions: DefaultThemeOptions = {
	...
	components: {
		...
		MuiTextField: {
			defaultProps: {
				// Inputs use the brand-primary (violet) focus colour so the
				// whole form aligns with the Sign-in button and "+New
				// Project" CTA. Earlier this defaulted to 'secondary' (cyan)
				// which clashed with the brand and gave the form a "wrong
				// product" feel. Explicit color="secondary" on individual
				// fields still works for intentional cyan opt-ins.
				color: 'primary',
				size: 'medium'
			}
		},
		...
		MuiOutlinedInput: {
			...
			styleOverrides: {
				root: {
					// Previous values were minHeight 32px + 6px/12px padding,
					// which collapsed the field into a squat strip — too
					// short to read float-label state at a glance and tight
					// for touch targets. 48px is the standard MUI medium
					// density and matches the WCAG 2.5.5 44px touch-target
					// minimum.
				...
				input: {
					padding: `${spacing(12)} ${spacing(14)}` // 12px 14px — breathing room for the floating label
				},
				inputSizeSmall: {
